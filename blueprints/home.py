    ...
    for tx in txs:
        if tx.tx_type == '지출':
            ...
            if c_name not in cat_expense_total:
                ...
                day_expense_by_cat[c_name] = [0]*days_in_month
...
def build_home_data(user_id, y, m):
    ...
    # 지출분석 제외 -> 요일별/일별/카테고리별 분석 차트에서 빠짐
    # 예산에서만 제외 -> 이번 달 요약(총수입/총지출)과 예산 진행률(및 참여자 구성비)에서 빠짐
    analysis_txs = [tx for tx in month_tx if not tx.exclude_analysis]
    budget_txs = [tx for tx in month_tx if not tx.exclude_budget]

    monthly_income = sum(tx.amount for tx in budget_txs if tx.tx_type == '수입')
